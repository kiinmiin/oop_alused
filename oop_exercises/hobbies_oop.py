...
def filter_by_hobby(people_list: list, hobby: str) -> list:
    ...
    filtered_list = list(filter(lambda person: hobby in person.hobbies, people_list))
    sorted_list = sorted(filtered_list, key=lambda person: person.first_name)
    print('1', filtered_list)
    print('2', sorted_list)
    return sorted_list
    pass
...
def sort_by_most_hobbies(people_list: list) -> list:
    ...
    """
    sorted_by_most_hobbies = sorted(people_list, key=lambda x: x.hobbies , reverse=True)
    return sorted_by_most_hobbies
    pass
...
def sort_by_least_hobbies(people_list: list) -> list:
    ...
    :return: sorted list of people.
    """
    sorted_by_least = sorted(people_list, key=lambda person: (len(person.hobbies), person.full_name))
    return sorted_by_least
    pass
...
def sort_people_and_hobbies(people_list: list) -> list:
    ...
    """
    for person in people_list:
        sorted_hobbies = sorted(person.hobbies)
        person.hobbies = sorted_hobbies
    people_sorted = sorted(people_list, key=lambda person: person.full_name)
    print('3', people_sorted)
